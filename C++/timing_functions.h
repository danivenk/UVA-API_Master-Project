...
vector<string> a = {"find_nearest", "calc_dispfrac", "calc_illumination_fracs",
                    "calc_timing_params", "calc_propagation_params",
                    "calc_radial_time_response", "calc_irfs_mono",
                    "calc_disk_band", "bb_phflux", "linear_rebin_irf",
                    "calc_cross_psd", "lorentz_q", "calculate_stprod_mono"};

// json to save the values in
json times_Cpp;

void init_json() {
    for (int i = 0; i < a.size(); i++) {
        times_Cpp[a.at(i)] = json::array();
    }
}
...
tuple<T, int> find_nearest(U<T> array, T value) {
    ...
    times_Cpp[__func__].push_back(duration.count());
