...
def calc_radial_time_response(rad,rcor,disp_frac,disktocor_frac,cortodisk_frac,seed_frac_flow,heat_frac_flow):
    ...
    lseed_disp = disp_frac*(disktocor_frac+seed_frac_flow)*(1-f_rev)
    lheat = disp_frac*heat_frac_flow*(1-f_rev)
    # The reverberation seed is just the luminosity returning to the corona
    lseed_rev = f_return*(1-f_rev)*disp_frac*(disktocor_frac+seed_frac_flow+heat_frac_flow)/(1-f_return)
    # Finally the reverberation which escapes the system without returning to the corona
    ldisk_rev = (f_rev-f_return)*disp_frac*(disktocor_frac+seed_frac_flow+heat_frac_flow)/(1-f_return)
...
def run_simulation(rad,iseed_start,nsims,nbins,expquery,multquery,dt,deltau_scale,nirf,segnbin,bfac,lfreq,q,rms,gamma_par,\
                   ...
                   disktocor_frac,cortodisk_frac,disk_abs_frac):
    ...
    for i in range(nsims):
        ...
        lc_gamma, mdot = sim_lcs(rad,iseed,dt,nbins,expquery,multquery,lfreq,q,rms,gamma_par,i_rsigmax,irf_nbins,\
            deltau_scale,rcor,disktocor_frac,cortodisk_frac,disk_abs_frac,disk_disp_irf,disk_outer,seed_disp_irf,\
        ...
        print("Total mean luminosity (absorption corrected): ",np.mean(lc_disk_disp+lc_seed_disp+lc_heat+(lc_disk_rev+lc_seed_rev)/disk_abs_frac))
...
def sim_lcs(rad,iseed,dt,nbins,expquery,multquery,lfreq,q,rms,gamma_par,i_rsigmax,irf_nbins,deltau_scale,rcor,\
            disktocor_frac,cortodisk_frac,disk_abs_frac,disk_disp_irf,disk_outer,seed_disp_irf,seed_outer,
            heat_irf,heat_outer):
    ...
    f_return = np.sum(cortodisk_frac*disktocor_frac) # the fraction of coronal luminosity that is reprocessed by the disk
    # and returned to the corona
    lc_seed_rev = disk_abs_frac*f_return*(lc_seed_disp+lc_heat)/(1-f_return)
    lc_disk_rev = disk_abs_frac*(f_rev-f_return)*(lc_seed_disp+lc_heat)/((1-f_return)*(1-f_rev))
