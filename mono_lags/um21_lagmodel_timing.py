...
times_python = collections.defaultdict(list)
...
def find_nearest(array,value):
    ...
    stop = time.time()
    times_python[func_name()].append(stop-start)
...
def calc_dispfrac_ind(rad,rad_area,rin,rcor,disp_ind,seedff_norm,seedff_ind,heatff_norm,heatff_ind):
    """ Calculates viscous dissipation for each disk annulus normalised by the total summed over all radii.
        Also calculates the fraction of dissipated power from radii inside the corona which go into heating
        and seed photons. The sum of heating and seed luminosity fraction is not automatically kept equal
        to 1, to allow for unconstrained heating/cooling, e.g. from energy transport to the flow from outside
    ...
    dispfrac = rad_area*(rad**(disp_ind))*(1.-np.sqrt(rin/rad))   # Assumes zero-torque inner boundary
    dispfrac = dispfrac/np.sum(dispfrac)
    seed_frac_flow = np.zeros(len(rad))
    heat_frac_flow = np.zeros(len(rad))
...
def inv_cone_diskgeom(r,params):
    """ Calculates (numerically) solid angle, illumination fractions for a (frustrum of) an inverted conical
    'solid' corona with base radius r_cor, height h_cor, radius at the top r_top, and assuming a disk with
    radially dependent thickness surface height h given by a power-law with normalisation h_0 and index h_ind.
    The model finds solid angle
    and illumination fractions by numerical integration using the approach given in Appendix in
    the UM20 paper. Number of cell spacings on vertical and azimuthal axes given by nz and nphi respectively
    (params[5] and params[6])"""
    r_cor = params[0]
    h_cor = params[1]
    r_top = params[2]
    h_0 = params[3]
    ...
    nphi = params[6]
    h = h_0*(r/r_cor)**h_ind
    h_grad = h_0*h_ind*(r_cor**(-h_ind))*(r**(h_ind-1))
    dz = h_cor/nz
    dphi = 2.*np.pi/nphi
    z = np.arange(0,h_cor,dz)
    phi = np.arange(0,2*np.pi,dphi)
    ...
    ypos = ypos/rpos
    zpos = zpos/rpos
    zpos_disk = 1/np.sqrt(1.+h_grad**2)
    xpos_disk = -h_grad/np.sqrt(1.+h_grad**2)
    daomega = np.multiply(da,np.multiply(x1,xpos)+np.multiply(y1,ypos)+np.multiply(z1,zpos))
    rcorpos = np.sqrt(np.square(r_cone)+np.square(z1))
    daomega = np.divide(daomega,(np.square(rpos)*rcorpos))
    daomegawt = np.multiply(daomega,np.multiply(xpos_disk,-1*xpos)+np.multiply(zpos_disk,-1*zpos))
    omega = daomega[daomega > 0].sum()
    omegawt = daomegawt[daomegawt > 0].sum()
    f_disktocor = omegawt/np.pi  # Fraction of disk photons from this radius which hit the corona
    f_cortodisk = f_disktocor/(np.pi*(r_top**2+(r_cor+r_top)*np.sqrt((r_top-r_cor)**2+h_cor**2))) # Fraction of
...
def run_stprod_mono_supp(rin,rout,nrad,rcor,rsigmax,seedff_norm,seedff_ind,heatff_norm,heatff_ind,cor_geometry,
                    geopar,thermal_frac,t_scale,disk_tau_par,cor_tau_par,lor_model,lor_par,
                    disk_prop_par,cor_prop_par,gamma_par,e_seed,ens_mono,encomb,nirf_mult,min_deltau_frac,var_suppress_fparams):
    ...
    var_suppress_params = []
    for par_list in var_suppress_fparams:
        [freq_supp,fdisk_supp,fseed_supp,fheat_supp] = par_list
        tau_supp = 1./(freq_supp*t_scale)
        tau_supp, i_supp = find_nearest(tau,tau_supp)
        var_suppress_params.append([rad[i_supp],fdisk_supp,fseed_supp,fheat_supp])
        print('Suppression frequency:',freq_supp,' for radius ',rad[i_supp],' R_g.')
    
    # Calculate IRFs for the mono-energetic case
    gamma_mean, irfs_list = calc_irfs_mono_supp(gamma_par,e_seed,ens_mono,ldisk_disp,lseed_disp,lheat,ldisk_rev,lseed_rev,var_suppress_params,rad)
    print("Mean gamma is:", gamma_mean)
    freq, phlag, tlag, psd_ci, psd_ref, mod_sig_psd, irf_nbins, irf_binedgefrac, deltau_scale, dt, nirf, ci_irfs_list =\
    calculate_stprod_mono_supp(nirf_mult,ens_mono,encomb,irfs_list,var_suppress_params,rad,deltau,min_deltau_frac,i_rsigmax,lfreq,q,rms,t_scale)
    
    return gamma_mean, freq, phlag, tlag, psd_ci, psd_ref, tau, i_rcor, i_rsigmax, rad, deltau, ldisk_disp,\
    lseed_disp, lheat, ldisk_rev, lseed_rev, irfs_list, mod_sig_psd
...
def run_stprod_mono_multicomp(rin,rout,nrad,rcor,rsigmax,seedff_norm,seedff_ind,heatff_norm,heatff_ind,cor_comp_list,
    ...
    rsigmax, i_rsigmax = find_nearest(rad,rsigmax)
    print("Maximum signal radius reset to: ",rsigmax)
    [r_comp,geom_comp,fd2c_scale,fc2d_scale,geopar_comp] = cor_comp_list
    ...
    for i in range(len(r_comp)):
        ...
        if r_comp_out > rcor:
            ...
            disktocor_frac_comp[i,:] = fd2c_scale[i]*disktocor_frac_comp[i,:]
            cortodisk_frac_comp[i,:] = fc2d_scale[i]*cortodisk_frac_comp[i,:]
            if r_comp_out < rcor:
            ...
            ldisk_disp_comp, lseed_disp_comp[i,:], lheat_comp[i,:], ldisk_rev2, lseed_rev2 =\
                calc_radial_time_response(rad,rcor,disp_frac,disktocor_frac_comp[i,:],
                                      cortodisk_frac_comp[i,:],thermal_frac,seed_frac_flow,heat_frac_flow)
            if r_comp_out == rcor:
...
def run_setup_mono_supp(rin,rout,nrad,rcor,rsigmax,seedff_norm,seedff_ind,heatff_norm,heatff_ind,cor_geometry,
                    geopar,thermal_frac,t_scale,disk_tau_par,cor_tau_par,lor_model,lor_par,
                    disk_prop_par,cor_prop_par,gamma_par,e_seed,ens_mono,encomb,nirf_mult,min_deltau_frac,var_suppress_fparams):
    ...
    return gamma_mean, rad, tau, i_rcor, i_rsigmax, deltau, ldisk_disp,\
    lseed_disp, lheat, ldisk_rev, lseed_rev, irfs_list, lfreq
...
def compare_disk_seed_centroids(rin,rout,nrad,rsigmax,seedff_norm,seedff_ind,heatff_norm,heatff_ind,geom_list,
    ...
    for i, x in enumerate(geom_list):
        ...
        run_setup_mono(rin,rout,nrad,rcor,rsigmax,seedff_norm,seedff_ind,heatff_norm,heatff_ind,cor_geometry,
                    geopar,thermal_frac,t_scale,disk_tau_par,cor_tau_par,lor_model,lor_par,
                    disk_prop_par,cor_prop_par,gamma_par,e_seed,ens_mono,encomb,nirf_mult,min_deltau_frac)
    ...
    for i, ax in enumerate((ax1, ax2)):
        ...
        if sigrad != None:
            for radius in sigrad:
                ...
                if ax == ax1:
                    radtxt = str(round(radius))+" $R_{g}$"
                    ax.text(lfreq[i_rsig],0.175,radtxt,fontsize=12,horizontalalignment='center')
